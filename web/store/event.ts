    ...
    .filter(
      (e) =>
        (!e.event.requires.length ||
          e.event.requires.every((r) => (r.startsWith('!') ? !states.includes(r.substring(1)) : states.includes(r)))) &&
        (additionalFilter?.(e) ?? true)
    )
}

export function selectOnGreetingEvent(entries: AppSchema.ScenarioEvent[], states: string[]) {
  const applicableEvents = filterApplicableEvents<AppSchema.ScenarioOnGreeting>(entries, 'onGreeting', states)
...
export function selectOnChatOpenedEvent(entries: AppSchema.ScenarioEvent[], states: string[], lastModified: Date) {
  const now = new Date()
...
export const eventStore = createStore<ChatEventState>('events', { events: [], preventLoop: false })((get, set) => {
  return {
    onGreeting(_, chat: AppSchema.Chat) {
      if (!chat.scenarioIds?.length) return
      try {
        const scenarios = scenarioStore.getState().scenarios.filter((s) => chat.scenarioIds?.includes(s._id))
        // Instructions
        scenarios
          .filter((s) => !!s.instructions)
          .forEach((s) => msgStore.queue(chat._id, `**Scenario: ${s.name}**\nInstructions: ${s.instructions}`, 'ooc'))
        // Greeting
    ...
    *onCharacterMessageReceived({ preventLoop }, chat: AppSchema.Chat, messagesSinceLastEvent: number) {
      ...
      const scenarios = scenarioStore.getState().scenarios.filter((s) => chat.scenarioIds?.includes(s._id))
      const entries = scenarios.flatMap((s) => s.entries)
      const event = selectOnCharacterMessageReceivedEvent(entries, chat.scenarioStates || [], messagesSinceLastEvent)
      if (event) {
    ...
    async triggerEvent(_, chat: AppSchema.Chat, char: AppSchema.Character) {
      ...
      const prompt = selected.event.text.replace(BOT_REPLACE, char.name).replace(SELF_REPLACE, profile?.handle || 'You')
      ...
      msgStore.send(eventState.chatId, eventState.prompt, ('send-event:' + eventState.event) as any)
...
function updateChatScenarioStates(chat: AppSchema.Chat, assigns: string[]) {
  ...
  if (assigns.length) {
    ...
    const updatedStates = Array.from(new Set(chatStates.filter((s) => !remove.includes(s)).concat(add)))
    const equivalent =
      chatStates.length === updatedStates.length && chatStates.every((val) => updatedStates.includes(val))
