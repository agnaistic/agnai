...
import HomePageAiFans from './pages/HomePage'
...
const Layout: Component<{ children?: any }> = (props) => {
  ...
  const containerDivStyles = createMemo(() => {
    if (isAiFansPage()) {
      return ''
    } else {
      return `mx-auto h-full min-h-full ${isChat() ? maxW() : 'max-w-8xl'} px-2 sm:px-3`
    }
  })

  const containerDivClasslist = createMemo(() => {
    if (isAiFansPage()) {
      return {}
    } else {
      return {
