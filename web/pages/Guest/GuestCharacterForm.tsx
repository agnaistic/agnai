...
const GuestCharacterForm: Component = () => {
  const { editId, duplicateId } = useParams()
  const state = guestStore((s) => ({
    edit:
      editId || duplicateId ? s.chars.find((ch) => ch._id === (editId || duplicateId)) : undefined,
  }))
  ...
  const onSubmit = (ev: Event) => {
    ...
    if (editId) {
      guestStore.editCharacter(editId, payload)
      nav('/guest/character/list')
    } else {
      guestStore.createCharacter(payload)
      nav('/guest/character/list')
    }
  }
...
export default GuestCharacterForm
