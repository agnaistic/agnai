...
export const Images: Component<{ state: ConfigState; setters: ConfigSetters }> = (props) => {
  const settings = settingStore((s) => s.config)
  const state = adminStore((s) => ({ config: s.config }))

  const models = createMemo(() => {
    if (!props.state?.imagesModels) return []

    const base = props.state.imagesModels.map((m) => ({ label: m.desc, value: m.id }))
  ...
  return (
    <Card bg="bg-800" class="flex flex-col gap-2" bgOpacity={1}>
      <TextInput
          ...
          <>
            <div class="flex gap-2">
              ...
              <Toggle
                ...
                onChange={(ev) => props.setters('imagesEnabled', ev)}
              />
            </div>
          </>
        }
        ...
        onChange={(ev) => props.setters('imagesHost', ev.currentTarget.value)}
      />
      ...
      <Select
        ...
        onChange={(ev) => props.setters('defaultImageModel', ev.value)}
      />

      <ImageModels state={props.state} setters={props.setters} />
    </Card>
  )
}

const ImageModels: Component<{ state: ConfigState; setters: ConfigSetters }> = (props) => {
  const rows = useRowHelper({
    getter: () => props.state.imagesModels,
    setter: (next) => props.setters('imagesModels', next),
    empty: () => ({
      id: v4().slice(0, 4),
      name: '',
      desc: '',
  ...
  return (
    <div class="flex flex-col gap-2">
      ...
      <div class="flex flex-col gap-3">
        <Index each={props.state.imagesModels}>
          {(item, i) => (
