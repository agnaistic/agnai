...
import PromptModal from '../../Chat/components/PromptModal'
import DeleteMsgModal from '../../Chat/DeleteMsgModal'
import { devCycleAvatarSettings, isDevCommand } from '../../Chat/dev-util'
import ForcePresetModal from '../../Chat/ForcePreset'
import DeleteChatModal from '../../Chat/components/DeleteChat'
import { useEffect, usePaneManager } from '/web/shared/hooks'
import { emptyMsg, InfiniteScroll, insertImageMessages, SwipeMessage } from '../../Chat/helpers'
...
import ChatPanes, { useValidChatPane } from '../../Chat/components/ChatPanes'
...
import ChatHeaderAiFans from './ChatHeaderAiFans'
    ...
    <div class="flex h-screen flex-col overflow-hidden">
      <div>
        <div class="flex flex-col">
          <div class="w-full  xl:container md:mx-auto md:px-5 lg:px-[20px] xl:px-[40px]  2xl:max-w-[1775px] 2xl:px-[60px]">
            <div class="m-3 ">
              <Header />
            </div>
          </div>
          <div class="h-[1px] w-full bg-[#949494]"></div>
          <ChatHeaderAiFans image={chats.char?.avatar} name={chats.char?.name} />
        </div>
      </div>
      <div class="flex-1 overflow-y-scroll">
        <section
          data-messages
          class={`flex w-full flex-1 flex-col-reverse gap-4`}
          ref={monitorScroll}
        >
          <div id="chat-messages" class="flex w-full flex-col gap-2">
            {/* Original Slot location */}
  ...
  return (
    <>
      <div class="flex min-h-screen flex-col justify-between">
        ...
        <section
          data-messages
          class={`mx-auto flex w-full flex-1 flex-col-reverse gap-4 overflow-y-scroll`}
          ref={monitorScroll}
        >
          <div id="chat-messages" class="flex w-full flex-col gap-2">
            {/* Original Slot location */}
            ...
            <Index
              each={[...chatMsgs(), ...chatMsgs(), ...chatMsgs(), ...chatMsgs(), ...chatMsgs()]}
            >
              {(msg, i) => (
                <>
                  <Message
