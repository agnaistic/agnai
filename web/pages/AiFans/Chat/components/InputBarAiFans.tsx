...
import { SpeechRecognitionRecorder } from '../../../Chat/components/SpeechRecognitionRecorder'
...
import TextInputAiFans from './TextInputAiFans'

const InputBarAiFans: Component<{
  chat: AppSchema.Chat
  ...
  return (
    <div class="relative flex items-end justify-center">
      ...
      <div class="flex flex-1 items-center overflow-hidden rounded-md border border-cosplay-gray-100 bg-[#1e2023] ">
        <TextInputAiFans
          fieldName="chatInput"
          isMultiline
          spellcheck
          ...
          parentClass="flex flex-1"
          class="input-bar flex-1 rounded-none bg-[#1e2023] font-clash hover:bg-[#1e2023] focus:bg-[#1e2023] "
          onKeyDown={(ev) => {
            if (ev.key === '@') {
              setComplete(true)
            }
          ...
          onInput={updateText}
        />
        <button onClick={onButtonClick} class="mr-3 rounded-md bg-[#404243] px-1 py-[6px]">
          <div class="flex items-center ">
            <img src="/images/generate-image-icon.png" class="h-[14px] w-[18px]" />
            <div class="px-[6px] font-clash text-xs">Ask</div>
            <img src="/images/chevron-icon-down.png" class="h-[7px] w-[10px]" />
          </div>
        </button>
      </div>
      ...
      <Switch>
        ...
        <Match when>
          <button
            onClick={send}
            class="ml-2 h-[43px] rounded-xl bg-gradient-to-r from-cosplay-purple to-cosplay-blue-200 px-3"
          >
            <img src="/images/send-button-icon.png" width="23" height="20" />
          </button>
        </Match>
      </Switch>
    </div>
